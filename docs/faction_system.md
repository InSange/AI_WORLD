...
## 2. 파벌 소속 유형 (Affiliation Type)

파벌은 **상위 파벌로부터 파생**될 수도 있고 **자연 발생적으로 독립**할 수도 있다.

| 소속 유형 | ID | 설명 | 예시 |
|-----------|-----|------|------|
| **독립** | `independent` | 상위 파벌 없음. 자체 생존 | 자연 발생 마을, 유목 부족 |
| **종속 (직할)** | `colony` | 상위 파벌이 직접 파견·지배 | 제국이 세운 전초기지, 국경 요새 |
| **봉신** | `vassal` | 자체 지도자 존재. 세금·군사 의무 | 왕국 하위 영지 |
| **동맹 연합** | `allied` | 소속은 아니지만 공동 이익 | 느슨한 도시 동맹 |

### 소속 관계 예시

```
중앙 제국 (empire) ← 최상위
  ├── 북부 총독령 (city,  colony)     ← 황제가 직접 임명
  │     ├── 철령 요새 (outpost, colony)  ← 악마·몬스터 차단 목적
  │     └── 강변 읍 (town,   vassal)   ← 자율 영주, 세금 납부
  ├── 리베르 왕국 (kingdom, vassal)    ← 독자 왕. 충성 서약만
  │     └── 항구 마을 (village, colony)
  └── 서부 방어선 (outpost, colony)    ← 드래곤/엘프 경계

자연 발생 마을 (village, independent)  ← 그냥 사람들이 모임
```

### 독립 파벌 발생 조건
- 인구 50 이상이 한 지역에 정착하면 자동으로 마을(village) 파벌 생성
- 부모 파벌로부터 이탈 이벤트 (반란, 인정, 고립)
- 탐험 중 오지에 정착

### 종속 파벌의 의무
- `colony`: 상위에 자원 25% 납부. 지도자는 상위가 임명
- `vassal`: 상위에 자원 15% 납부. 지도자는 자체 선출. 전쟁 참여 의무
